...
def mod_VIS(zetac, rho, u, v, model, Nc=20000, LLD=0., Na=8, tol=1e-5):
    ...
    """
    # contouring (depreciated)
    if model == 'ESPL_CONT_SINGEP' or model == 'ESPL_CONT_MULTIEP':
        Vp, Vm, Np, Nm = vismag(zetac, rho, u, v, Nc, Nc)
        V = (Vp + Vm) / (Np + Nm)
        
    # full integration
    elif model == 'ESPL_FULLINT_SINGEP' or model == 'ESPL_FULLINT_MULTIEP':
        V = fullvis(zetac, rho, u, v, LLD=LLD, Na=Na, tol=tol)
        
    # point-source approximation
    elif model == 'ESPL_PS_SINGEP' or model == 'ESPL_PS_MULTIEP':
        V = psvis(zetac, u, v)
        
    # flat-images approximation
    elif model == 'ESPL_FLAT_SINGEP' or model == 'ESPL_FLAT_MULTIEP' or model == 'ESPL_FLAT_MULTIEP_RESC' or model == 'ESPL_MLENSING_MULTIEP':
...
def mod_T3PHI(zetac, rho, u_AB, v_AB, u_BC, v_BC, model, Nc=20000, LLD=0., Na=8, tol=1e-5):
    ...
    for b in B:
        ...
        # full integration
            ...
            Vbis = Vbis * fullvis(zetac, rho, b[0], b[1], LLD=LLD, Na=Na, tol=tol)
...
def fullvis(zetac, rho, u, v,  LLD=0., Na=8, tol=1e-5):
    ...
    def F(fi, uE, vE):
        ...
        if betam == 0.:
            ...
            VpR = romberg(intfunc, -betam, betam, args=(1, u0, fi * eta, uE, vE), tol=tol)
            VpI = romberg(intfunc, -betam, betam, args=(2, u0, fi * eta, uE, vE), tol=tol)
            VmR = romberg(intfunc, -betam, betam, args=(-1, u0, fi * eta, uE, vE), tol=tol)
            VmI = romberg(intfunc, -betam, betam, args=(-2, u0, fi * eta, uE, vE), tol=tol)
            return (np.complex(VpR, VpI) - np.complex(VmR, VmI))
